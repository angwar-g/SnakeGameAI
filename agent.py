...
class Agent:
    ...
    def __init__(self):
        ...
        self.memory = deque(maxlen=MAX_MEMORY)    # if memory exceeds, popleft() is called
        self.model = Linear_QNet(11, 256, 3)  # 11 features, 3 actions
    ...
    def remember(self, state, action, reward, next_state, gameover):
        self.memory.append((state, action, reward, next_state, gameover)) # popleft() if MAX_MEMORY is exceeded

    def train_long_memory(self):
        if len(self.memory) > BATCH_SIZE:
            mini_sample = random.sample(self.memory, BATCH_SIZE)    # random.sample() returns a list of tuples
        else:
